...
impl RustshotGui {
    ...
    fn gesture_toolbox_buttons(&self) {
        ...
        //////////////////////////////////////////////////////////////
        // Helper that implements the shared “stop‑drawing / start‑drawing”
        // logic used by several toolbar buttons.
        //////////////////////////////////////////////////////////////
        fn toggle_drawing<F>(
            button: &gtk::Button,
            subwin: &gtk::Window,
            boxes: &std::rc::Rc<std::cell::RefCell<DrawingAreaManager>>,
            handles: &std::rc::Rc<std::cell::RefCell<Handles>>,
            toolbox: &std::rc::Rc<std::cell::RefCell<Toolbox>>,
            ...
            F: FnOnce(),
        {
            ...
            if pressed.get() {
                ...
                if button.css_classes().iter().any(|c| c == "pressed") {
                    button.remove_css_class("pressed");
                    return;
                }
        ...
        set_btn.connect_clicked(glib::clone!(
            ...
            move |button| {
                // -------------------------------------------------------------
                // 1️⃣  If we are already in “drawing” mode, stop it.
                // -------------------------------------------------------------
                if pressed.get() {
                    // Restore UI state.
                    pressed.set(false);
                    ...
                    // If the clicked button already carried the “pressed” CSS class,
                    // we simply toggle it off and exit early.
                    if button.css_classes().iter().any(|c| c == "pressed") {
                        button.remove_css_class("pressed");
                        return;
                    }
                ...
                // -------------------------------------------------------------
                // 2️⃣  Enter “drawing” mode for the newly‑clicked button.
                // -------------------------------------------------------------
                button.add_css_class("pressed");
                pressed.set(true);
        ...
        add_numbered_circles.connect_clicked(glib::clone!(
            ...
            pressed,
            move |b| {
                toggle_drawing(b, &subwin, &boxes, &handles, &toolbox, &pressed, || {
