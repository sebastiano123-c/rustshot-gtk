...
glib::wrapper! {
    pub struct NumberedCircleSettingsBox(ObjectSubclass<imp::NumberedCircleSettingsBox>)
        @extends gtk::Box,
        @implements gtk::Accessible,  gtk::Actionable, gtk::Widget, gtk::Orientable, gtk::Buildable, gtk::ConstraintTarget;
}

impl Default for NumberedCircleSettingsBox {
    fn default() -> Self {
        glib::Object::new()
    }
}

impl NumberedCircleSettingsBox {
    ...
    pub fn populate_with_settings(&self, geom: &GeometryState) -> std::io::Result<()> {
        ...
        let settings = geom.settings.numbered_circle.clone();
        let imp = &self.imp();
            ...
            move |s| {
                ...
                if sz.abs() > 0 && sz.signum() > 0 {
                    settings
                        ...
                        .expect("Error in NumberedCircleSettingButton");
                }
            }
        ));
        ...
        // radius size
        let radius_spin = IntegerInput::new(settings.get_value("radius").get_f64()? as i32, 0, 100);
        radius_spin.connect_value_changed(glib::clone!(
            #[strong]
            settings,
            move |s| {
                let sz = s.value();
                if sz.abs() > 0 && sz.signum() > 0 {
                    settings
                        .set_value("radius", SettingValue::F64(sz as f64))
                        .expect("Error in NumberedCircleSettingButton");
                }
            }
        ));
        radius_spin.set_tooltip_text(Some("change radius"));
        ...
        // number size
        // imp.number_spin = IntegerInput::new(settings.get_value("number").get_i32()?, 0, 100);
        imp.number_spin.connect_value_changed(glib::clone!(
            #[strong]
            settings,
            move |s| {
                let sz = s.value();
                if sz.abs() > 0 && sz.signum() > 0 {
                    settings
                        .set_value("number", SettingValue::I32(sz))
                        .expect("Error in NumberedCircleSettingButton");
                }
            }
        ));
        imp.number_spin.set_tooltip_text(Some("change number"));
        ...
        let number_box: gtk::Box = gtk::Box::new(gtk::Orientation::Horizontal, 4);
        number_box.append(&imp.number_spin);
        crate::toolbox_settings_box::add_expandable_row(
            self,
            "\u{31}",
            "starting number",
    ...
    pub fn update_number(&self, value: i32) -> std::io::Result<()> {
        ...
        imp.number_spin.set_value(value);
        Ok(())
    }
}
