...
impl ArcSettingsBox {
    ...
    pub fn populate_with_settings(&self, geom: &GeometryState) -> std::io::Result<()> {
        // get arc settings
        let settings = geom.settings.arc.clone();
            ...
            move |s| {
                ...
                if sz.abs() > 0 && sz.signum() > 0 {
                    settings
                        ...
                        .expect("Error in ArcSettingButton");
                }
            }
        ));
